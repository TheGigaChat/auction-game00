import React from "react";

function Container10Default() {
  return (
    <section className="Container10Default displayNone" id="Container10Default">
      <section className="items">
        <section className="printerSection">
          <p>+199$</p>
          <img src={require("./img/container10img/printer.png")} alt="reload" />
        </section>
        <section className="gamingTable1Section">
          <p>+99$</p>
          <img
            src={require("./img/container10img/gamingTable1.png")}
            alt="reload"
          />
        </section>
        <section className="laptop1Section">
          <p>+799$</p>
          <img src={require("./img/container10img/laptop1.png")} alt="reload" />
        </section>
        <section className="markerBoardSection">
          <p>+89$</p>
          <img
            src={require("./img/container10img/markerBoard.png")}
            alt="reload"
          />
        </section>
        <section className="gamingTable2Section">
          <p>+189$</p>
          <img
            src={require("./img/container10img/gamingTable2.png")}
            alt="reload"
          />
        </section>
        <section className="laptop2Section">
          <p>+899$</p>
          <img src={require("./img/container10img/laptop2.png")} alt="reload" />
        </section>
        <section className="wifiRuuterSection">
          <p>+39$</p>
          <img
            src={require("./img/container10img/wifiRuuter.png")}
            alt="reload"
          />
        </section>
      </section>
    </section>
  );
}

export default Container10Default;
