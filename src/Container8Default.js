import React from "react";

function Container8Default() {
  return (
    <section className="Container8Default displayNone" id="Container8Default">
      <section className="items">
        <section className="iphone15Section">
          <p>+1379$</p>
          <img src={require("./img/container8img/iphone15.png")} alt="reload" />
        </section>
        <section className="electricScooterSection">
          <p>+399$</p>
          <img
            src={require("./img/container8img/electricScooter.png")}
            alt="reload"
          />
        </section>
        <section className="christmasTreeSection">
          <p>+329$</p>
          <img
            src={require("./img/container8img/christmasTree.png")}
            alt="reload"
          />
        </section>
        <section className="gamingChairSection">
          <p>+219$</p>
          <img
            src={require("./img/container8img/gamingChair.png")}
            alt="reload"
          />
        </section>
        <section className="PS5Section">
          <p>+449$</p>
          <img src={require("./img/container8img/PS5.png")} alt="reload" />
        </section>
        <section className="moetSection">
          <p>+69$</p>
          <img src={require("./img/container8img/moet.png")} alt="reload" />
        </section>
        <section className="winterScarfSection">
          <p>+4$</p>
          <img
            src={require("./img/container8img/winterScarf.png")}
            alt="reload"
          />
        </section>
      </section>
    </section>
  );
}

export default Container8Default;
